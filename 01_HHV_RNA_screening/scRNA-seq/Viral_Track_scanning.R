...
for (k in List_target_path) {
  ...
  STAR_mapping_command = paste("STAR --runThreadN",N_thread,"--genomeDir",Index_genome,"--readFilesIn",k,"--outSAMattributes NH HI AS nM NM XS ","--outFileNamePrefix",name_prefix,"--outSAMtype BAM SortedByCoordinate","--twopassMode Basic ","--outFilterMatchNmin 25 --outFilterScoreMinOverLread 0.5 --outFilterMatchNminOverLread 0.5")
...
for (k in List_output_path) {
  ...
  detected_virus = rownames(QC_result[QC_result$Sequence_entropy >1.0 & QC_result$Longest_contig>2*Mean_mapping_length & QC_result$Spatial_distribution >0.01,])
